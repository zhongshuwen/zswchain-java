...
import org.zhongshuwen.zswjava.error.session.TransactionBroadCastEmptySignatureError;
import org.zhongshuwen.zswjava.error.session.TransactionBroadCastError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestAbiError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestEmptyAvailableKeyError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestKeyError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestRequiredKeysEmptyError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestRpcError;
import org.zhongshuwen.zswjava.error.session.TransactionCreateSignatureRequestSerializationError;
import org.zhongshuwen.zswjava.error.session.TransactionGetSignatureDeserializationError;
import org.zhongshuwen.zswjava.error.session.TransactionGetSignatureError;
import org.zhongshuwen.zswjava.error.session.TransactionGetSignatureNotAllowModifyTransactionError;
import org.zhongshuwen.zswjava.error.session.TransactionGetSignatureSigningError;
import org.zhongshuwen.zswjava.error.session.TransactionPrepareError;
import org.zhongshuwen.zswjava.error.session.TransactionPrepareInputError;
import org.zhongshuwen.zswjava.error.session.TransactionPrepareRpcError;
import org.zhongshuwen.zswjava.error.session.TransactionProcessorConstructorInputError;
import org.zhongshuwen.zswjava.error.session.TransactionSendTransactionError;
import org.zhongshuwen.zswjava.error.session.TransactionSerializeError;
import org.zhongshuwen.zswjava.error.session.TransactionSignAndBroadCastError;
import org.zhongshuwen.zswjava.error.session.TransactionSignError;
...
import org.zhongshuwen.zswjava.utilities.DateFormatter;
import org.zhongshuwen.zswjava.utilities.Utils;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

/**
 * Wrapper class for an ZSW Transaction.  This class allows the developer to use the Transaction
 * object in several ways.
 * <p>
 * The TransactionProcessor allows the developer to:
 * <p>
 * - Get input {@link org.zhongshuwen.zswjava.models.rpcProvider.Action}
 * <p>
 * - Create transaction
 * <p>
...
public class TransactionProcessor {
    ...
    /**
     * Transaction instance that holds all data relating to an ZSW Transaction.
     * <p>
    ...
    /**
     * Sign the transaction by passing {@link ZswChainTransactionSignatureRequest} to signature
     * provider
     * <p>
    ...
    public boolean sign() throws TransactionSignError {
        ZswChainTransactionSignatureRequest zswhqTransactionSignatureRequest;
        try {
            zswhqTransactionSignatureRequest = this.createSignatureRequest();
        ...
        ZswChainTransactionSignatureResponse zswhqTransactionSignatureResponse;
        try {
            zswhqTransactionSignatureResponse = this.getSignature(zswhqTransactionSignatureRequest);
            if (zswhqTransactionSignatureResponse.getError() != null) {
                throw zswhqTransactionSignatureResponse.getError();
            }
    ...
    public SendTransactionResponse signAndBroadcast() throws TransactionSignAndBroadCastError {
        ...
        try {
            this.getSignature(zswhqTransactionSignatureRequest);
        } catch (TransactionGetSignatureError transactionGetSignatureError) {
     ...
     *          calling {@link IABIProvider#getAbi(String, ZSWName)} to get the ABI needed to
     ...
     */
    @NotNull
    private ZswChainTransactionSignatureRequest createSignatureRequest()
        ...
        ZswChainTransactionSignatureRequest zswhqTransactionSignatureRequest = new ZswChainTransactionSignatureRequest(
        ...
        if (this.requiredKeys != null && !this.requiredKeys.isEmpty()) {
            zswhqTransactionSignatureRequest.setSigningPublicKeys(this.requiredKeys);
            return zswhqTransactionSignatureRequest;
        }
    ...
    /**
     * Passing {@link ZswChainTransactionSignatureRequest} to signature provider for signing.
     * <p>
     * Check getSignature() flow in "Complete Workflow" documents for more details.
     *
     * @param zswhqTransactionSignatureRequest The request for signature
     * @return Response from Signature provider
     */
    @NotNull
    private ZswChainTransactionSignatureResponse getSignature(
            ZswChainTransactionSignatureRequest zswhqTransactionSignatureRequest)
            throws TransactionGetSignatureError {
        ZswChainTransactionSignatureResponse zswhqTransactionSignatureResponse;
        try {
            zswhqTransactionSignatureResponse = this.signatureProvider
                    .signTransaction(zswhqTransactionSignatureRequest);
            if (zswhqTransactionSignatureResponse.getError() != null) {
                throw zswhqTransactionSignatureResponse.getError();
            }
        ...
        if (Strings.isNullOrEmpty(zswhqTransactionSignatureResponse.getSerializedTransaction())) {
            throw new TransactionGetSignatureSigningError(
                    ErrorConstants.TRANSACTION_PROCESSOR_SIGN_TRANSACTION_TRANS_EMPTY_ERROR);
        }

        if (zswhqTransactionSignatureResponse.getSignatures().isEmpty()) {
            throw new TransactionGetSignatureSigningError(
        ...
        if (this.serializedTransaction != null
                && !this.serializedTransaction
                .equals(zswhqTransactionSignatureResponse.getSerializedTransaction())) {
            ...
            try {
                transactionJSON = this.serializationProvider
                        .deserializeTransaction(
                                zswhqTransactionSignatureResponse.getSerializedTransaction());
        ...
        this.signatures.addAll(zswhqTransactionSignatureResponse.getSignatures());
        this.serializedTransaction = zswhqTransactionSignatureResponse.getSerializedTransaction();
        return zswhqTransactionSignatureResponse;
    }
     ...
     *          occurs while calling {@link IABIProvider#getAbi(String, ZSWName)} to get the ABI
    ...
    private AbiEosSerializationObject setupAbiEosSerializationObject(Action action, String chainId, IABIProvider abiProvider)
        ...
        try {
            actionAbiJSON = abiProvider
                    .getAbi(chainId, new ZSWName(action.getAccount()));
    ...
    /**
     * Gets transaction instance which holds all data relating to ZSW Transaction
     * <p>
     ...
     * <p>
     * Check getSignature() flow in "Complete Workflow" document for more details.
     * @return list of ZSW format signature.
     */
    @NotNull
