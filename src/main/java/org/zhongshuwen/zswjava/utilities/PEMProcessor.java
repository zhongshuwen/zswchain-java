...
public class PEMProcessor {
    ...
    public AlgorithmEmployed getAlgorithm() throws PEMProcessorError {
        ...
        } else if (GMObjectIdentifiers.sm2p256v1.getId().equals(oid)) {
    ...
    /**
     * Extract ZSW public key
     *
     * @param isLegacy - Set to true if the legacy format of the key is desired.  This uses "ZSW"
     ...
     * @return ZSW format public key of the current private key
     * @throws PEMProcessorError when the public key extraction fails.
     */
    public String extractZSWPublicKeyFromPrivateKey(boolean isLegacy) throws PEMProcessorError {
        ...
        try {
            return ZSWFormatter.encodePublicKey(publicKeyByteArray, keyCurve, isLegacy);
     ...
     *
     * @param isLegacy Whether to return the legacy format of the key.  This uses "ZSW"
     * to prefix the key data and only applies to keys generated with the secp256k1 algorithm.  The
     * new format prefixes the key data with "PUB_K1_".
     * @return ZSW format public key of the current private key
    ...
    public String extractPEMPublicKeyFromPrivateKey(boolean isLegacy) throws PEMProcessorError {
        try {
            return ZSWFormatter.convertZSWPublicKeyToPEMFormat(extractZSWPublicKeyFromPrivateKey(isLegacy));
